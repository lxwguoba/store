...
public class BankInfoActivity extends BaseAppActivity {
            ...
            switch (msg.what) {
                ...
                case 5:
                    try {
                        ...
                        if (status == 1) {
                            MessageShow.initErrorDialog(BankInfoActivity.this, jsonObject.getString("msg"), 1);
                            AppSharePreferenceMgr.put(BankInfoActivity.this, "sub_type_name", tlist.get(spPosition).getName());
                            AppSharePreferenceMgr.put(BankInfoActivity.this, "sub_type_id", tlist.get(spPosition).getValue());
                        } else {
                            MessageShow.initErrorDialog(BankInfoActivity.this, jsonObject.getString("msg"), 0);
                        }
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (store_id.length() == 0) {
            MessageShow.initErrorDialog(this, "请先增加店铺，再填写此信息。", 1);
        }
    }
...
//               /添加数组资源,跟组对应的位置
                ...
                BankInfoActivity.this.provincePos = position;
                BankInfoActivity.this.cityPos = 0;
                if (countys.length > 0) {
                    BankInfoActivity.this.areaPos = 0;
                } else {
                    BankInfoActivity.this.areaPos = -1;
                }
        ...
        city.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            ...
            public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                ...
                county_adapter.addAll(countys);
                county.setSelection(0);
                BankInfoActivity.this.cityPos = position;
                if (countys.length > 0) {
                    BankInfoActivity.this.areaPos = 0;
                } else {
                    BankInfoActivity.this.areaPos = -1;
        ...
        county.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            ...
            public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                ...
                BankInfoActivity.this.areaPos = position;
            }
    ...
    private void gotoSaveInfo() {
        ...
        String bank_addressInfo = addressInfo.getText().toString().trim();
        if (!(linkman_name.length() > 0)) {
            MessageShow.initErrorDialog(BankInfoActivity.this, "请输入联系人姓名", 0);
            return;
        }
        if (!(linkman_phone.length() > 0)) {
            MessageShow.initErrorDialog(BankInfoActivity.this, "请输入联系人电话", 0);
            return;
        }
        if (!(linkman_email.length() > 0)) {
            MessageShow.initErrorDialog(BankInfoActivity.this, "请输入联系人邮箱", 0);
            return;
        }
        if (!(bank_UserName.length() > 0)) {
            MessageShow.initErrorDialog(BankInfoActivity.this, "请输入开户人姓名", 0);
            return;
        }
        if (!(bank_AccountName.length() > 0)) {
            MessageShow.initErrorDialog(BankInfoActivity.this, "请输入开户人身份证", 0);
            return;
        }

        if (!(bank_Account.length() > 0)) {
            MessageShow.initErrorDialog(BankInfoActivity.this, "请输入银行卡账号", 0);
            return;
        }
        if (!(bank_OpenName.length() > 0)) {
            MessageShow.initErrorDialog(BankInfoActivity.this, "请输入开户行全称", 0);
            return;
        }


//        if (!(bank_addressInfo.length() > 0)) {
//            MessageShow.initErrorDialog(BankInfoActivity.this, "请输入开户行详细地址", 0);
//            return;
//        }
        ...
        String userName = UserConfig.getUserName(BankInfoActivity.this);
