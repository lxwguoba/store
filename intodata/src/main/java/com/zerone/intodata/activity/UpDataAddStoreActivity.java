...
import com.zerone.intodata.domain.UPStoreInfoBean;
...
public class UpDataAddStoreActivity extends BaseAppActivity {
    @Bind(R.id.titleName)
    TextView titleName;
    ...
    Handler handler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                ...
                case 5:
                    String stroeinfo = (String) msg.obj;
                    try {
                        JSONObject jsonObject = new JSONObject(stroeinfo);
                        int status = jsonObject.getInt("status");
                        if (status == 1) {
                            String data = jsonObject.getString("data");
                            Gson gson = new Gson();
                            UPStoreInfoBean upStoreInfoBean = gson.fromJson(data, UPStoreInfoBean.class);
                            gotoupview(upStoreInfoBean);
                        }
                    } catch (Exception e) {
                        e.printStackTrace();
                    }
                ...
                case 6:
                    try {
                        ...
                        int status = jsonObject.getInt("status");
                        if (status == 1) {
                            MessageShow.initErrorDialog(UpDataAddStoreActivity.this, jsonObject.getString("msg"), 0);
                        }
     ...
     * 获取到的数据修改页面
     *
     * @param upStoreInfoBean
     */
    private void gotoupview(UPStoreInfoBean upStoreInfoBean) {
        storeName.setText(upStoreInfoBean.getName());
        storePhone.setText(upStoreInfoBean.getMobile());
        bankOpenName.setText(upStoreInfoBean.getRealname());
        storeDiscount.setText(upStoreInfoBean.getThirdpay_rate());
        for (int i = 0; i < hclist.size(); i++) {
            if (upStoreInfoBean.getIndustry_id().equals(hclist.get(i).getId() + "")) {
                storeCatering.setSelection(i);
                caterPosition = i;
                break;
            }
        }
        for (int i = 0; i < optionsPro.size(); i++) {
            if (upStoreInfoBean.getProvince_id() == optionsPro.get(i).getId()) {
                provincePos = i;
...
//               /添加数组资源,跟组对应的位置
        ...
        for (int j = 0; j < coptionsCity.size(); j++) {
            if (upStoreInfoBean.getCity_id() == coptionsCity.get(j).getId()) {
                cityPos = j;
                city.setSelection(j);
                initArea(j);
                break;
        ...
        for (int l = 0; l < coptionsArea.size(); l++) {
            if (upStoreInfoBean.getArea_id() == coptionsArea.get(l).getId()) {
                areaPos = l;
    ...
    private void initPageData() {
        ...
        NetUtils.netWorkByMethodPost(this, parmar, IPConfig.GET_STORE_DETAIL, handler, 5);
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_up_addstore);
    ...
    private void gotoAddStore() {
        ...
        if (!(s_name.length() > 0)) {
            MessageShow.initErrorDialog(UpDataAddStoreActivity.this, "请输入商户名称", 0);
            return;
        }
        if (!(s_phone.length() > 0)) {
            MessageShow.initErrorDialog(UpDataAddStoreActivity.this, "请输入手机号码", 0);
            return;
        }
        if (!(s_password.length() > 0)) {
            MessageShow.initErrorDialog(UpDataAddStoreActivity.this, "请设置密码", 0);
            return;
        }
        if (!(s_real_name.length() > 0)) {
            MessageShow.initErrorDialog(UpDataAddStoreActivity.this, "请输入真实姓名", 0);
            return;
        }
        if (!(s_discount.length() > 0)) {
            MessageShow.initErrorDialog(UpDataAddStoreActivity.this, "请输入联盟折扣", 0);
            return;
        }
        ...
        String timestamp = System.currentTimeMillis() + "";
        String token = CreateTokenUtils.createToken(this, timestamp);
        String userName = UserConfig.getUserName(UpDataAddStoreActivity.this);
        parmar.put("timestamp", timestamp);
        parmar.put("username", userName);
        parmar.put("token", token);
        ...
        zLoadingDialog.show();
        NetUtils.netWorkByMethodPost(this, parmar, IPConfig.STORE_APPLY_UPDATE, handler, 6);
    }
...
//               /添加数组资源,跟组对应的位置
                ...
                UpDataAddStoreActivity.this.provincePos = position;
                UpDataAddStoreActivity.this.cityPos = 0;
                if (countys.length > 0) {
                    UpDataAddStoreActivity.this.areaPos = 0;
                } else {
                    UpDataAddStoreActivity.this.areaPos = -1;
                }
            }
        ...
        city.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            ...
            public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                ...
                county_adapter.addAll(countys);
                county.setSelection(0);
                UpDataAddStoreActivity.this.cityPos = position;
                if (countys.length > 0) {
                    UpDataAddStoreActivity.this.areaPos = 0;
                } else {
                    UpDataAddStoreActivity.this.areaPos = -1;
        ...
        county.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            ...
            public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                ...
                UpDataAddStoreActivity.this.areaPos = position;
            }
